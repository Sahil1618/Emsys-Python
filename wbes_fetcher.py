...
def _export_results_to_excel(results: list, date_str: str) -> None:
    ...
    Files are saved under:
        WBES/<region>/<YYYY-MM-DD>/WBES_BlockWise_<qca>_<date>_Rev<rev>.xlsx
    """
    try:
        from wbes.config import QCA_REGIONS
    ...
    for result in results:
        ...
        file_path = result.get("file_saved")
        ...
        try:
            ...
            region = QCA_REGIONS.get(qca_name, "UNKNOWN")
            path   = export_schedule_to_excel(rows, qca_name, iso_date, region)
